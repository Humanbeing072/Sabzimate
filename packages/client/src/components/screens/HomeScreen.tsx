...
import { Language, Vegetable, OrderItem, ParsedOrderItem, User, translations, getTodaysVegetables, sendDeliveryConfirmation, sendVoiceOrder, LoadingSpinner } from 'common';
...
const HomeScreen: React.FC<HomeScreenProps> = ({ language, searchQuery, user }) => {
  ...
  const handleDeliveryChoice = (choice: 'YES' | 'NO') => {
    ...
    sendDeliveryConfirmation(user.id, choice);
  };
  ...
  const processTranscription = async (text: string) => {
    ...
    try {
        ...
        }).filter((item): item is { id: string; quantity: string } => item !== null);
