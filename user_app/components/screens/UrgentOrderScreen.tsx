...
import { Language, Vegetable, OrderItem, User } from '../../../common/types';
import { translations } from '../../../common/constants';
import { getTodaysVegetables, placeUrgentOrder } from '../../../common/api';
...
const UrgentOrderScreen: React.FC<UrgentOrderScreenProps> = ({ language, searchQuery, user }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
        const itemsToOrder = order.map(item => {
            const veg = vegetables.find(v => v.id === item.id);
            return { veg, quantity: item.quantity };
        }).filter(item => item.veg) as { veg: Vegetable, quantity: string }[];

        await placeUrgentOrder(user, itemsToOrder);
