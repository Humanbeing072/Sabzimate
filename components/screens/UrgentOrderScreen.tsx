...
import { Language, Vegetable, OrderItem } from '../../common/types';
import { translations } from '../../common/constants';
import { getTodaysVegetables } from '../../common/api';
...
const UrgentOrderScreen: React.FC<UrgentOrderScreenProps> = ({ language, searchQuery }) => {
    ...
    <div className="p-4">
      ...
      {isLoading ? <LoadingSpinner /> : (
        <>
          ...
          <div className="mt-8">
            <button className="w-full bg-green-600 text-white font-bold py-3 rounded-lg shadow-md hover:bg-green-700 transition-colors text-lg">
              {t.placeOrder} ({order.length} {language === Language.EN ? 'items' : 'आइटम'})
            </button>
          </div>
        </>
      )}
